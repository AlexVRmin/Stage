<script>
    // @ts-nocheck
      ...
      let contract = school.Contract;
      let hoofdcontractLookup = data.hoofdcontractLookup;
    </script>
      ...
      <section class="flex justify-between">
        ...
        <aside class="w-1/5">
          ...
          <div class="bg-white px-9 py-4 flex flex-col gap-6">
            <div>
              ...
              <span class="ml-1">{school.Voornaam} {school.Admin}</span>
            </div>
    
            <div>
              <h3 class="font-bold">Primary Email</h3>
              <span class="ml-1">{school.Email}</span>
            </div>
            ...
            <div>
              <h3 class="font-bold">Schoolgroup</h3>
              <span class="ml-1">
        ...
        <section class="w-3/4">
          ...
          <div class="bg-white px-9 py-4">
            {#each contract as contracten}
              {#if hoofdcontractLookup[contracten.HoofdcontractFK]}
    
                <strong>{hoofdcontractLookup[contracten.HoofdcontractFK].Naam}</strong>
              {/if}
              <div class="record">
                  <span class="w-48">{contracten.Naam}</span> 
                  <span class="w-40">{contracten.PCN}</span> 
                  <span class="w-40">{contracten.Startdatum}</span> 
                  <span class="w-40">{contracten.Einddatum}</span> 
                  <a class="px-4 py-0.5 bg-edutech-orange rounded-2xl" href={`/Licences/${contracten.PKcontract}`}>View</a>
              </div>
