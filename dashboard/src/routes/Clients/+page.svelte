<script>
  ...
  function Telefoon() {
    ...
    alert('Wie tf sorteert er op telefoon');
  }
...
<main class="m-9 flex flex-col gap-2">
  ...
  <section class="border-solid border-2 border-gray-500 rounded-lg flex px-12 mb-8">
    ...
    <span class="w-72 cursor-pointer" on:click={Groep}>Schoolgroup</span>
    <span class="w-72 cursor-pointer" on:click={Telefoon}>Phone</span>  <!--style-->
    <span class="w-72 cursor-pointer" on:click={Email}>Email</span>    <!--style-->
  ...
  {#each sckool as school}
    <div class="record">
      ...
      {/if}
      <span class="w-40">{school.Telefoonnummer}</span> <!--style-->
      <span class="w-40">{school.Email}</span>           <!--style-->
